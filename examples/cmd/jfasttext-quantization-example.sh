...
JFASTTEXT_CMD="java -jar ../../target/jfasttext-*-jar-with-dependencies.jar"

echo "Training..."
${JFASTTEXT_CMD} supervised -input "${DATADIR}/dbpedia.train" -output "${RESULTDIR}/dbpedia" -dim 10 -lr 0.1 -wordNgrams 2 -minCount 1 -bucket 10000000 -epoch 5 -thread 4

echo "Quantizing..."
${JFASTTEXT_CMD} quantize -output "${RESULTDIR}/dbpedia" -input "${DATADIR}/dbpedia.train" -qnorm -retrain -epoch 1 -cutoff 100000

echo "Testing original model..."
${JFASTTEXT_CMD} test "${RESULTDIR}/dbpedia.bin" "${DATADIR}/dbpedia.test"
echo "Testing quantized model..."
${JFASTTEXT_CMD} test "${RESULTDIR}/dbpedia.ftz" "${DATADIR}/dbpedia.test"
